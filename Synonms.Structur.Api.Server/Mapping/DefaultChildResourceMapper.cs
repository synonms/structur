    ...
    where TChildResource : ChildResource, new()
{
    ...
    private object? CreateValueObjectResourceFromValueObject(Type valueObjectType, Type valueObjectResourceType, object? valueObjectValue)
    {
        ...
        foreach (PropertyInfo valueObjectResourcePropertyInfo in valueObjectResourceProperties)
        {
            ...
            object? convertedValue = ConvertValue(valueObjectPropertyInfo.PropertyType, valueObjectResourcePropertyInfo.PropertyType, valueObjectPropertyValue);
    ...
    private object? ConvertValue(Type fromType, Type toType, object? fromValue)
    {
        if (fromValue is null) return null;
        
        if (fromType.IsValueObject())
        {
            PropertyInfo? valueObjectValuePropertyInfo = fromType.GetProperty("Value", BindingFlags.Instance | BindingFlags.Public);
            ...
            object? rawValue = valueObjectValuePropertyInfo.GetValue(fromValue);
            
            // ValueObject<string> to Enum e.g. AddressType -> AddressTypeEnumeration
            if (fromType.GetValueObjectValueType() == typeof(string) && toType.IsEnum)
            {
